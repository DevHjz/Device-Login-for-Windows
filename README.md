...
安全认证结果仅按五项检查中**明确发现的安全隐患**计算：没有明确隐患显示**通过检测**；出现 1–2 项隐患显示**存在隐患**；出现超过 2 项明确问题显示**高危风险**。系统暂未返回某一检查项时仅保留在详情中，不计入风险，也不会显示“检测异常”。

## 五、桌面状态悬浮窗

默认显示“云端验证设备认证状态”悬浮窗，默认尺寸为 **200 × 200 px**。可在系统设置中关闭或重新开启；窗口位置与大小会保存，支持手动拖动标题区域和在边缘调整尺寸。系统设置可用像素数值**分别设置宽度与高度**，范围均为 160–720 px；还可设置 35%–100% 的**背景透明度**，文字和图标始终保持完全不透明，并可锁定悬浮窗以禁止移动和调整大小。悬浮窗包含下列内容：
...
| 安全认证 | 显示安全结果与网络访问状态，例如“通过检测 \| 公网访问”或“通过检测 \| 内网访问”；公网访问为黄色，内网访问为绿色；鼠标悬停显示五项检查详情。 |
...
```text
cloud-verify-device-auth-3.0.7-win-x64-setup.exe
cloud-verify-device-auth-3.0.7-win-arm64-setup.exe
```
