...
默认显示“云端验证设备认证状态”悬浮窗，默认尺寸为 **215 × 190 px**。可在系统设置中关闭或重新开启；窗口位置与大小会保存，支持手动拖动标题区域和在边缘调整尺寸。系统设置可用像素数值**分别设置宽度与高度**，范围均为 160–720 px；还可设置 0%–100% 的**背景透明度**，其中 0% 只隐藏浅蓝背景，文字和图标始终保持完全不透明，并可锁定悬浮窗以禁止移动和调整大小。悬浮窗包含下列内容：
