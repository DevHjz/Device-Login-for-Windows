...
| 检查项 | 检测方式 | 风险说明 |
| --- | --- | --- |
| 设备登录凭据 | 读取 Windows 本机密码/凭据策略 | 未检测到登录凭据要求时标为隐患。 |
| 杀毒软件 | 读取 Windows Security Center 已注册产品及启用状态 | 适用于 Defender、360、火绒等向 Security Center 注册的产品。 |
| 病毒库 | 读取已注册产品状态；Defender 可读取最近签名更新日期 | 超过 7 天或明确过期时标为隐患。 |
| Windows 防火墙 | 读取各网络配置文件启用状态 | 任一配置文件未启用时标为隐患。 |
| 网络 | 读取优先局域网 IPv4，并探测到 `1.1.1.1:443` 的出站连通性 | 可联通时显示“（公网接入）”。 |

安全认证结果按照四项检查计算：全部通过显示**通过检测**；出现 1–2 项隐患或无法确认的状态显示**存在隐患**；出现超过 2 项问题显示**高危风险**。
...
| 安全认证 | 绿色“通过检测”、黄色“存在隐患”、红色“高危风险”；鼠标悬停显示四项检查详情。 |
...
```text
cloud-verify-device-auth-3.0.2-win-x64-setup.exe
cloud-verify-device-auth-3.0.2-win-arm64-setup.exe
```
