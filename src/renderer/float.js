(() => {
  ...
  function riskCopy(report) {
    ...
    if (report.issueCount === 0) return { text: '通过检测', detail: '未发现需要处理的安全风险。系统暂未返回的检查项目不会计入风险。', color: '#176b3e' }
    if (report.risk === 'danger') return { text: '高危风险', detail: `发现 ${report.issueCount} 项明确的安全问题。`, color: '#a22f42' }
    return { text: '存在隐患', detail: `发现 ${report.issueCount} 项需要处理的安全问题。`, color: '#8a6116' }
  }
